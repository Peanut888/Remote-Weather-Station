 ...
 * Date: 26/04/24
 * Description: Transmit humidity, temprature, pressure, altitude, and air quality data via Lo-Ra radio every 10s.
*/
...
void loop() {
  ...
  if (now.second() % 10 == 0) // Transmit data every 10 seconds.
  {
    ...
    */
    ...
    dataLen = sprintf((char *)data, "%f %f %f %f %f", humi.relative_humidity, temp.temperature, (float)(bmp388.pressure / 100.0), (float)bmp388.readAltitude(SEA_LEVEL_PRESSURE_HPA), (float)sgp40.getVoclndex());
    /*
    *-----------------------------------------------------------------------------------------------------------------------------------------------*
