/*
Title: Weather Station Transmittter Experiment 6
Programmer: Justin Lam
Date: 20/02/23
Description: Experiment to test RTC, payload acknowledgement and transmitting temperature, humidity, pressure, wind speed, VOC index and if it is raining.
*/
...
bool success = false; // Success of transmissions.
...
void setup() {
  ...
  pinMode(transmitLedPin, OUTPUT); // Set the transmit debug LED to digital pin 27 and define it as an output.
  pinMode(failureLedPin, OUTPUT); // Set the failure to transmit debug LED to digital pin 27 and define it as an output.
  pinMode(retryLedPin, OUTPUT); // Set the retrying to transmit debug LED to digital pin 27 and define it as an output.
  ...
  while (!SGP40.begin()) { // Start operation of the sensor, needs at least 10s to heat the chip.
...
void loop() {
  ...
  if (curSecond != prevSecond && !isStart) {
  ...
  if ((now.second() % 5) == 0 && !isStart) { // Start measuring and transmitting data every 5 seconds if the transmitter has not just booted.
    ...
    data.pressure = BMP388.readPressPa() + 1150; // Get the atmospheric pressure.
    ...
    success = radio.write(&data, sizeof(dataPackage));
    delay(250);
    checkTransmission(success, numRetry);
...
void checkTransmission(bool status, int retry) {
  ...
  if (status) {
    if (radio.isAckPayloadAvailable()) {   
      radio.read(&received, sizeof(acknowledge));
    ...
    status = radio.write(&data, sizeof(dataPackage));    
    digitalWrite(retryLedPin, LOW);
